...
export const updateGoal = async (
  ...
  // Description Update - Handles empty strings
  if (body.description !== undefined) {
    updates.description = body.description?.trim() ? sanitizeString(body.description) : null;
  }

  // Deadline Update - Prevents "Invalid Date" 400 errors
  if (body.deadline !== undefined) {
    if (body.deadline && body.deadline.toString().trim() !== "") {
      const date = new Date(body.deadline);
      if (isNaN(date.getTime())) {
        throw new AppError('Invalid date format provided for deadline', 400);
      }
  ...
  if (body.category !== undefined) {
    updates.category = body.category?.trim() ? sanitizeString(body.category) : null;
  }
...
export const deleteGoal = async (
  ...
  await Session.updateMany({ goalId: id, userId }, { $set: { goalId: null } });
...
export const archiveGoal = async (
  ...
  if (goal.pinned) goal.pinned = false;
  await goal.save();
...
export const togglePin = async (
  ...
  const goal = await Goal.findOne({ _id: id, userId });
  if (!goal) throw new AppError('Goal not found', 404);
  if (goal.status === 'archived') throw new AppError('Cannot pin archived goal', 400);

  if (!goal.pinned) {
    const pinnedCount = await Goal.countDocuments({ userId, pinned: true });
    if (pinnedCount >= 3) throw new AppError('Maximum 3 goals can be pinned.', 400);
  }
...
export const toggleMilestone = async (
  ...
  if (goal.progress < 100 && goal.status === 'done') goal.status = 'active';

  await goal.save();

  res.status(200).json({
    success: true,
    message: milestone.completed ? 'Milestone completed.' : 'Milestone uncompleted.',
    data: { goal },
  });
};
